...
export async function shareInvitation(): Promise<void> {
  try {
    ...
    // The share thumbnail is a 1080×1920 portrait file generated by
    // scripts/build-og-thumbnail.mjs — it composites the user's 1:1
    // master onto a 9:16 cream canvas. KakaoTalk's chat preview card
    // crops landscape and square images to its banner aspect ratio,
    // but renders portrait images at their full natural height. So
    // shipping a portrait file is the only way to guarantee the 1:1
    // design appears completely uncropped.
    //
    // Bump the ?v= every time scripts/build-og-thumbnail.mjs is re-run
    // so Kakao's CDN drops its cache (it caches OG images for hours).
    const imageUrl = `${siteUrl}/photos/og-thumbnail.jpg?v=4`;
